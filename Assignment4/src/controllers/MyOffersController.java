...
public class MyOffersController implements ActionListener {
	...
	public void actionPerformed(ActionEvent arg0) {
		if (arg0.getActionCommand().equals("View offer")) {
			...
			int option = JOptionPane.showConfirmDialog(null, query, "Add a characteristic", JOptionPane.OK_CANCEL_OPTION);
			if (option == JOptionPane.CANCEL_OPTION) {
				...
				case HOLIDAY:
					...
					holidayStartingDateField.setTodayButtonVisible(true);holidayStartingDateField.setWeekOfYearVisible(false);holidayStartingDateField.setDate(Date.from(ModifiableDate.getModifiableDate().atStartOfDay().atZone(ZoneId.systemDefault()).toInstant()));
					...
					String[] zipCodes = ((Host)(App.getLoggedUser())).getHouses().stream().map(House::getZipCode).collect(Collectors.toList()).toArray(new String[0]);
					...
					int holidayOption = JOptionPane.showConfirmDialog(null, holidayQuery, "Create a holiday offer", JOptionPane.OK_CANCEL_OPTION);
					if (holidayOption == JOptionPane.CANCEL_OPTION) {
						JOptionPane.showMessageDialog(null, "Operation cancelled succesfully");
					} else if (holidayOption == JOptionPane.OK_OPTION) {
						if (holidayPriceField.equals("") || holidayDepositField.equals("") || holidayDescriptionField.equals("")) {
							JOptionPane.showMessageDialog(null, "Please fill in all the fields before clicking this button", "Error", JOptionPane.ERROR_MESSAGE);
						...
						try {
							LocalDate startingDate = Instant.ofEpochMilli(holidayStartingDateField.getDate().getTime()).atZone(ZoneId.systemDefault()).toLocalDate();
							...
							LocalDate finishDate = Instant.ofEpochMilli(holidayFinishDateField.getDate().getTime()).atZone(ZoneId.systemDefault()).toLocalDate();
							
							createdOffer = this.app.createHolidayOffer(startingDate, price, deposit, description, offeredHouse, finishDate);
							
						} catch (NumberFormatException e) {
							JOptionPane.showMessageDialog(null, "Please enter price and deposit in a suitable format, i.e. 0.25 or 34.21", "Error", JOptionPane.ERROR_MESSAGE);
				...
				case LIVING:
					...
					livingStartingDateField.setTodayButtonVisible(true);livingStartingDateField.setWeekOfYearVisible(false);livingStartingDateField.setDate(Date.from(ModifiableDate.getModifiableDate().atStartOfDay().atZone(ZoneId.systemDefault()).toInstant()));
					...
					String[] livingZipCodes = ((Host)(App.getLoggedUser())).getHouses().stream().map(House::getZipCode).collect(Collectors.toList()).toArray(new String[0]);
					JComboBox<String> livingHouseField = new JComboBox<String>(livingZipCodes);
					JTextField livingDuration = new JTextField();
					Object[] livingQuery = {
							"Starting date:", livingStartingDateField,
							"Price:", livingPriceField,
							"Desposit:", livingDepositField,
							"Description:", livingDescriptionField,
							"Offered house:", livingHouseField,
							"Duration:", livingDuration							
					};
					
					int livingOption = JOptionPane.showConfirmDialog(null, livingQuery, "Create a living offer", JOptionPane.OK_CANCEL_OPTION);
					if (livingOption == JOptionPane.CANCEL_OPTION) {
						JOptionPane.showMessageDialog(null, "Operation cancelled succesfully");
					} else if (livingOption == JOptionPane.OK_OPTION) {
						if (livingPriceField.equals("") || livingDepositField.equals("") || livingDescriptionField.equals("") || livingDuration.equals("")) {
							JOptionPane.showMessageDialog(null, "Please fill in all the fields before clicking this button", "Error", JOptionPane.ERROR_MESSAGE);
							this.actionPerformed(arg0);
							return;
						}
